...
$output = Join-Path $repoRoot "artifacts\runtime-feature-registry-test"
...
try {
    ...
    & $compiler /nologo /W4 /O2 /EHsc @includes `
        (Join-Path $repoRoot "runtime\RuntimeFeatureRegistry.cpp") `
        (Join-Path $repoRoot "tests\RuntimeFeatureRegistryTests.cpp") `
        "/Fo$output\" "/Fe:$output\RuntimeFeatureRegistryTests.exe" /link @libraries
    if ($LASTEXITCODE -ne 0) {
        throw "Runtime feature parser test build failed with exit code $LASTEXITCODE"
    }
    & (Join-Path $output "RuntimeFeatureRegistryTests.exe")
    if ($LASTEXITCODE -ne 0) {
        throw "Runtime feature parser tests failed with exit code $LASTEXITCODE"
    }
}
finally {
