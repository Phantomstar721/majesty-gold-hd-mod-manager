...
$output = Join-Path $repoRoot "artifacts\runtime-capability-manifest-test"
...
try {
    ...
    & $compiler /nologo /W4 /O2 /EHsc @includes `
        (Join-Path $repoRoot "runtime\RuntimeCapabilityManifest.cpp") `
        (Join-Path $repoRoot "tests\RuntimeCapabilityManifestTests.cpp") `
        "/Fo$output\" "/Fe:$output\RuntimeCapabilityManifestTests.exe" /link @libraries
    if ($LASTEXITCODE -ne 0) {
        throw "Runtime capability parser test build failed with exit code $LASTEXITCODE"
    }
    & (Join-Path $output "RuntimeCapabilityManifestTests.exe")
    if ($LASTEXITCODE -ne 0) {
        throw "Runtime capability parser tests failed with exit code $LASTEXITCODE"
    }
}
finally {
