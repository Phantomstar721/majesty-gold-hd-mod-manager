...
$output = Join-Path $repoRoot "artifacts\intent-text-registry-test"
...
try {
    ...
    & $compiler /nologo /W4 /O2 /EHsc @includes `
        (Join-Path $repoRoot "runtime\IntentTextRegistry.cpp") `
        (Join-Path $repoRoot "tests\IntentTextRegistryTests.cpp") `
        "/Fo$output\" "/Fe:$output\IntentTextRegistryTests.exe" /link @libraries
    if ($LASTEXITCODE -ne 0) {
        throw "Intent-text parser test build failed with exit code $LASTEXITCODE"
    }
    & (Join-Path $output "IntentTextRegistryTests.exe")
    if ($LASTEXITCODE -ne 0) {
        throw "Intent-text parser tests failed with exit code $LASTEXITCODE"
    }
}
finally {
