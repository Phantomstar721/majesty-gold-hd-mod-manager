...
class ResolvedQuestBoardRecord:
    ...
    child_dialog_id: int
    open_command_id: int
    selected_action_command_id: int
    refresh_command_id: int
    refresh_control_id: int
    refresh_price_binding_id: int
...
# Quest-board Refresh chrome is generated by the Manager from MX05's literal
# selected-action row. These IDs are therefore Manager-owned output, not author
# input. They may repeat across different child dialogs because Majesty scopes
# control lookup to the active streamed panel.
QUEST_REFRESH_CONTROL_ID = 0x7102
QUEST_REFRESH_PRICE_BINDING_ID = 0x7103
QUEST_REFRESH_COIN_CONTROL_ID = 0x7104
    ...
    *,
    ...
    for item in panel_features:
        ...
        if isinstance(item, StockAp10Ap69SecondaryPanel):
            ...
            command_base = 0x20000 + len(quest_boards) * 2
            quest_boards.append(ResolvedQuestBoardRecord(
                item.panel_key, parent, child, item.open_command_id,
                command_base, command_base + 1, QUEST_REFRESH_CONTROL_ID,
                QUEST_REFRESH_PRICE_BINDING_ID, item.list_source_callback_symbol,
                item.revision_callback_symbol, item.offer_name_callback_symbol,
                item.offer_goal_callback_symbol, item.offer_reward_callback_symbol,
                item.selected_cost_callback_symbol,
                item.selected_action_callback_symbol, item.refresh_cost_callback_symbol,
                item.can_refresh_callback_symbol, item.refresh_callback_symbol,
...
def encode_stock_controller_registry(registry: ResolvedControllerRegistry) -> bytes:
    """Encode canonical recipes as MMCR v2/v3/v4/v7 as features require."""
...
def decode_stock_controller_registry(payload: bytes) -> ResolvedControllerRegistry:
    """Decode canonical MMCR v2/v3/v4/v7 registries."""
    ...
    if version == 7:
        if len(payload) < _QUEST_HEADER.size:
            raise ControllerRegistryError("MMCR v7 header is truncated")
        magic, version, *counts = _QUEST_HEADER.unpack_from(payload)
        if counts[11] == 0:
            raise ControllerRegistryError("MMCR v7 without quest lists is noncanonical")
    elif version == 4:
...
def _encode_feature(writer: _Writer, feature: object) -> None:
    ...
    if isinstance(feature, ResolvedSecondaryPanelRecord):
        ...
        writer.u32(feature.open_command_id)
        for value in (
            feature.selected_action_command_id, feature.refresh_command_id,
            feature.refresh_control_id, feature.refresh_price_binding_id,
        ):
            writer.u32(value)
        for symbol in (
            feature.list_source_callback_symbol, feature.revision_callback_symbol,
            feature.offer_name_callback_symbol, feature.offer_goal_callback_symbol,
            ...
            feature.can_refresh_callback_symbol, feature.refresh_callback_symbol,
        ):
...
def _decode_feature(reader: _Reader, kind: Type[ControllerFeature]) -> object:
    ...
    if kind is StockAp08Mx05QuestBoardPanel:
        ...
        selected_action = reader.u32("selected_action_command_id")
        refresh_command = reader.u32("refresh_command_id")
        refresh_control = reader.u32("refresh_control_id")
        refresh_price = reader.u32("refresh_price_binding_id")
        symbols = [reader.symbol("quest-list callback") for _ in range(10)]
        return ResolvedQuestBoardRecord(
            panel, parent, child, opened, selected_action, refresh_command,
            refresh_control, refresh_price, *symbols,
            reader.fourcc("parent_controller_base"),
        )
...
def _validate_resolved_registry(
    ...
    for index, item in enumerate(sorted(registry.quest_boards, key=lambda p: p.panel_key)):
        ...
        expected_base = 0x20000 + index * 2
        if (item.selected_action_command_id != expected_base or
                item.refresh_command_id != expected_base + 1):
            raise ControllerRegistryError("MMCR quest-board commands are not manager-allocated")
        if item.parent_controller_base != "AP08":
            raise ControllerRegistryError(
                "MMCR quest-board parent controller base must be AP08"
            )
        if (
            item.refresh_control_id != QUEST_REFRESH_CONTROL_ID
            or item.refresh_price_binding_id != QUEST_REFRESH_PRICE_BINDING_ID
        ):
            raise ControllerRegistryError(
                "MMCR quest-board Refresh controls are not manager-allocated"
            )
        child_dialogs.add(item.child_dialog_id)
        parent_commands.add(parent_command)
        ...
        author_features.append(StockAp08Mx05QuestBoardPanel(
            item.panel_key, _resolved_parent_key(item.parent_dialog_id),
            _unpack_fourcc(item.child_dialog_id), item.open_command_id,
            item.list_source_callback_symbol, item.revision_callback_symbol,
            item.offer_name_callback_symbol, item.offer_goal_callback_symbol,
