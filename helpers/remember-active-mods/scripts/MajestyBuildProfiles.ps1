function Get-MajestyBuildProfiles {
    return @(
        ...
        [pscustomobject]@{
            Id = "gog-1.5.2.28"
            DisplayName = "GOG (1.5.2.28)"
            StockSha256 = "65C6DD32C3D873C2E320BDAA2DE1B00488AF85B44573FD0FD82F79A2FFD37792"
            TimeDateStamp = [uint32]0x5BBB8DB8
            BaseSections = @(
                @(".text",  0x001000, 0x000400, 0x34C000, 0x34BEFD),
                @(".rdata", 0x34D000, 0x34C400, 0x084400, 0x0843F8),
                @(".data",  0x3D2000, 0x3D0800, 0x00D400, 0x05908C),
                @(".rsrc",  0x42C000, 0x3DDC00, 0x001000, 0x000F34)
            )
            LoadCallOffset = 0x0766C0
            LoadCallVa = 0x4772C0
            SaveCallOffset = 0x137FE9
            SaveCallVa = 0x538BE9
            ApplyActiveModsVa = 0x5393A0
            ModsOkHandlerVa = 0x539240
            ActiveListInsertVa = 0x536190
            ActiveListCommitVa = 0x536240
            FprintfIat = 0x74D414
            FopenIat = 0x74D458
            FreadIat = 0x74D45C
            FcloseIat = 0x74D46C
            SscanfIat = 0x74D47C
            InstalledListVa = 0x7E0C34
            ActiveListOwnerVa = 0x7E0C3C
            ActiveListVa = 0x7E0C50
            ActiveListDirtyVa = 0x7E0C00
            PatchSectionCharacteristics = [uint32]3758096416 # 0xE0000020
            UsePrivateScratchArena = $true
            OriginalLoadCall = [byte[]]@(0xE8, 0x7B, 0x1F, 0x0C, 0x00)
            OriginalSaveCall = [byte[]]@(0xE8, 0xB2, 0x07, 0x00, 0x00)
        }
    )
}
...
function Assert-GogStockCallees {
    ...
    try {
        $ranges = @(
            @(1279904, 489, "6ED9FD10B7AEC9F19B5F84991A11254B3B14B3D0D72BF784914A97A9A350E3FD"),
            @(1279552, 338, "7C355E4F9AF16882585A10E1EEF6A0F8EB279862431CB4AB313C333C6EBFBA62"),
            @(1278624, 136, "8CB5C2B82062178D4F0722000BA9D787B041441939715A9006B88097B6F04A1B"),
            @(1267088, 168, "0E0359E129814E1E037A0A7F04F92ADAFB5E3024ACC0DCE981742B1DA660FAEE"),
            @(1267264, 130, "E42BB2716967A90CA328F8781CC5D731C43A4C90093BE5D01D6D751FBAA5F21E")
        )
