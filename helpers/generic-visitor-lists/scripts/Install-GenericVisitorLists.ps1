...
} elseif ($buildProfile.Key -eq "beta2") {
    $GateOffset = 0x98005
...
} elseif ($buildProfile.Key -eq "gog") {
    $GateOffset = 0x985B5
    $IconDispatchOffset = 0x983F8
    $IconDispatchVa = 0x498FF8
    $ThreatDispatchOffset = 0x986B1
    $ThreatDispatchVa = 0x4992B1
    $MonsterResolverOffset = 0xBB4E0
    $MonsterResolverVa = 0x4BC0E0
    $DisplayClassifierOffset = 0x10A270
    $DisplayClassifierVa = 0x50AE70
    $AttributeGetterOffset = 0x1CD6C0
    $AttributeGetterVa = 0x5CE2C0
    $IconHeroResumeVa = 0x499001
    $IconMonsterResumeVa = 0x49905D
    [byte[]]$OriginalThreatDispatchBytes = @(0xE8, 0x0A, 0x50, 0x13, 0x00)
    [byte[]]$MonsterResolverSignature = @(0x53, 0x55, 0x56, 0x8B, 0x74, 0x24, 0x10, 0x57, 0x6A, 0x01, 0x8B, 0xCE, 0xE8, 0x9F, 0xAB, 0x1C, 0x00, 0x68, 0x49, 0x58, 0x39, 0x32)
    [byte[]]$DisplayClassifierSignature = @(0x6A, 0xFF, 0x68, 0x04, 0x56, 0x71, 0x00, 0x64, 0xA1, 0x00, 0x00, 0x00, 0x00, 0x50, 0x83, 0xEC)
    [byte[]]$AttributeGetterSignature = @(0x8B, 0x54, 0x24, 0x04, 0x8D, 0x44, 0x24, 0x04, 0x50, 0x52, 0x83, 0xC1, 0x04, 0xE8, 0xFE, 0x9E)
    $supportsLegacyPatch = $false
} else {
    throw "No audited visitor profile for this executable."
}
$pe = Get-PeInfo $bytes
