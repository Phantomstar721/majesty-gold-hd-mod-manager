...
. (Join-Path $PSScriptRoot "GogQuestSection.ps1")
...
$gogSection = Initialize-GogQuestSection $bytes
if ($null -ne $gogSection) {
    $bytes = $gogSection.Bytes
    $profile = Get-GogUnlockProfile $gogSection
} else {
...
if ($exePatched) {
    ...
    if ($null -ne $gogSection) { $bytes = Remove-GogQuestSection $bytes }
