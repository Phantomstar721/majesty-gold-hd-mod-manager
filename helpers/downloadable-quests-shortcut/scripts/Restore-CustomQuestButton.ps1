...
$BuildProfiles = @(
    ...
    [pscustomobject]@{
        ...
        MinimumLength = 4058112
        SectionHeaderSha256 = "2986FD63DAA2836FBE20521241446CDB718644BA9B1108D830B92FCED92A6DBF"
        FreestyleIconCallbackOffset = 0x78296
        FreestyleCallbackBytes = [byte[]]@(0xE0,0x7C,0x47,0x00)
        CustomQuestCallbackBytes = [byte[]]@(0xE0,0x7B,0x47,0x00)
        CustomQuestCompareImmediateOffset = 0x78C19
        LegacyUiDataHashes = @{}
    }
)
...
function Restore-UiDataOwnedFields {
    ...
    if ($InspectOnly) {
        # Planning still performs the complete structural validation and builds
        # the exact result.  Retain it so the write phase does not parse every
        # current/backup archive a second time after all targets are validated.
        return [pscustomobject]@{
            Status = "WouldRestore"
            Path = $Path
            PreparedBytes = $newBytes
        }
    }
...
function Get-MajestyBuildProfile {
    ...
    if ($null -eq $profile) {
        throw ("Unsupported MajestyHD.exe build. Detected FileVersion '{0}', PE timestamp 0x{1:X8}. Supported builds are Steam Default Public 1.5.2.24, beta2 1.5.2.28, and GOG 1.5.2.28." -f $fileVersion, $timeDateStamp)
    }
    ...
    if ($profile.Id -eq "gog-1.5.2.28") { $callbackPrefix[2] = 0x74 }
...
foreach ($record in $backupRecords) {
    ...
    if ($plan.Status -eq "AlreadyRestored") {
        ...
        if ($null -eq $plan.PreparedBytes -or $plan.PreparedBytes.Length -eq 0) {
            throw "Prepared restore data is missing for $($record.Name). Refusing to modify it."
        }
        [IO.File]::WriteAllBytes($target, [byte[]]$plan.PreparedBytes)
