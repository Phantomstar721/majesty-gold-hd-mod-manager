...
function Get-QuestMapDragProfiles {
    ...
    return @(
        ...
        [pscustomobject]@{
            ...
            BuildSignatureOffset = 0x77FB0
            BuildSignatureBytes = [byte[]]@(0x6A,0xFF,0x68,0xD8,0x1B,0x70,0x00,0x64,0xA1,0x00,0x00,0x00,0x00,0x50,0x83,0xEC,0x24,0x53,0x55,0x56,0x57)
            HookOffset = 0x789C9
            CaveOffset = 0x34C300
            OriginalHookBytes = [byte[]]@(0x8B,0x44,0x24,0x48,0x8B,0x54,0x24,0x24)
            HookBytes = [byte[]]@(0xE9,0x32,0x39,0x2D,0x00,0x90,0x90,0x90)
            StubBytes = [Convert]::FromBase64String(
                "YGoB/xXI1HQAZqkAgA+EjgAAAIt0JAxorLCCAGiosIIA6HrN7/+DxAihqLCCADtGeHxuO4aAAAAAf2ahrLCCADtGfHxcO4aEAAAAf1SBPaCwggBEUkFHdRyhpLCCACsFqLCCAAFGSKGwsIIAKwWssIIAAUYkoaiwggCjpLCCAKGssIIAo7CwggDHBaCwggBEUkFHYYtEJEiLVCQk6TDG0v/HBaCwggAAAAAA6+Y="
            )
        }
    )
}
...
function Get-MajestyBuildProfile {
    ...
    if ($matches.Count -ne 1) {
        throw "MajestyHD.exe is not a supported build. Supported builds: Steam Default Public 1.5.2.24, beta2 1.5.2.28, and GOG 1.5.2.28."
    }
    return $matches[0]
}
...
if (-not $hookAlreadyPatched -and -not $hookIsStock) {
    throw ("MajestyHD.exe is not the expected game build near file offset 0x{0:X}, or another patch already owns this hook." -f $HookOffset)
}
